...
pub fn init_vdr() -> VcxResult<()> {
    ...
    vdr::ping(&vdr, &json!(namespace_list).to_string())
        .wait()
        .map_err(|err|
            match err.error_code.clone() {
                ErrorCode::PoolLedgerTimeout => {
                    err.to_vcx(VcxErrorKind::PoolLedgerConnect,
                               format!("Can not connect to Pool \"{:?}\".", namespace_list))
                }
                ErrorCode::PoolIncompatibleProtocolVersion => {
                    err.to_vcx(VcxErrorKind::PoolLedgerConnect,
                               format!("Pool \"{:?}\" is not compatible with Protocol Version.", namespace_list))
                }
