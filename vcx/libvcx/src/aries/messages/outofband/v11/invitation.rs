...
use crate::aries::utils::normalize_service_keys;
use crate::aries::messages::outofband::SUPPORTED_HANDSHAKE_PROTOCOL;
...
//            self.handshake_protocols.push(MessageFamilies::Outofband.id());
    ...
    pub fn normalize_service_keys(&mut self) -> VcxResult<()> {
        normalize_service_keys(&mut self.services)
    }
}
...
pub mod tests {
    ...
    pub fn _invitation_did_formatted() -> Invitation {
        ...
        Invitation {
            ...
            handshake_protocols: vec![_handshake_protocol()],
            request_attach: attachment,
            services: vec![_service_did_formatted()],
            profile_url: None,
            public_did: None,
            ..Invitation::default()
        }
    ...
    fn test_outofband_invitation_build_works() {
        ...
        let expected = r#"{"@id":"testid","@type":"https://didcomm.org/out-of-band/1.1/invitation","goal":"To issue a Faber College Graduate credential","goal_code":"issue-vc","handshake_protocols":["https://didcomm.org/connections/1.0"],"label":"Faber College","requests~attach":[{"@id":"request-0","data":{"base64":"eyJyZXF1ZXN0Ijp7fX0="},"mime-type":"application/json"}],"services":[{"id":"VsKV7grR1BUE29mG2Fm2kX","priority":0,"recipientKeys":["GJ1SzoWzavQYfNL9XkaJdrQejfztN4XqdsiV4ct3LXKL"],"routingKeys":["Hezce2UWMZ3wUhVkh2LfKSs8nDzWwzs2Win7EzNN3YaR","3LYuxJBJkngDbvJj4zjx13DBUdZ2P96eNybwd2n9L9AU"],"serviceEndpoint":"http://localhost:8080","type":""}]}"#;
    ...
    fn test_outofband_invitation_from_json() {
        ...
        let invitation: Invitation = serde_json::from_str(&invite).unwrap();
        assert!(invitation.services.len() > 0);
        assert!(invitation.request_attach.0.len() > 0);
    }
    ...
    pub fn test_normalize_service_keys_works() {
        ...
        assert_eq!(_service(), invitation.services[0])
    }
}